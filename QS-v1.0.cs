...
/*
   Cleaned & fixed version of QuantumSignals
   - Fixed parameter name mix-ups (Bollinger deviations)
   - Replaced invalid comparison operators
   - Corrected Stochastic threshold logic
   - Rewrote Bollinger band checks to be directionally meaningful
   - Trailing stop & Break-even now use parameters instead of hardcoded values
   - Break-even loop no longer returns early for all positions
